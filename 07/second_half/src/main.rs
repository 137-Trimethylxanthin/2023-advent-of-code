...
fn score_hand(hand: &str) -> (HandType, (u32, u32, u32, u32, u32)) {
    ...
    let mut best_hand_type = HighCard;
    let mut best_card_scores = (0, 0, 0, 0, 0);

    if hand.contains('J') {
        for card in ['A', 'K', 'Q', 'T', '9', '8', '7', '6', '5', '4', '3', '2'].iter() {
            let replaced_hand = hand.replace('J', &card.to_string());
            let counts = replaced_hand.chars().counts();
            ...
            if hand_type > best_hand_type {
                ...
                best_card_scores = card_scores;
            }
        }
    } else {
        let counts = hand.chars().counts();
        let values = counts.values().sorted().join("");
        best_hand_type = match values.deref() {
            "5" => FiveOfAKind,
            "14" => FourOfAKind,
            "23" => FullHouse,
            "113" => ThreeOfAKind,
    ...
    (best_hand_type, best_card_scores)
}
